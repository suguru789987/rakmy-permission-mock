...
| TC-P2-03 | 操作=一体：操作を許可 | 設定UIで店長 vendor=操作 | 保存→再取得 | vendor の登録・編集・削除すべて許可（3状態＝一体・別トグルなし） |
| TC-P2-04 | 操作=なし | 設定UIで店長 vendor=なし | 保存→再取得 | 登録/編集/削除すべてOFF |
| TC-P2-05 | 削除 DELETE /vendors/:id | U-staff1（vendor=なし） | 削除実行 | 403（操作なし＝削除不可） |
| TC-P2-06 | 削除（操作あり）＋実行時確認 | U-manager1（vendor=操作） | 削除実行→確認ダイアログOK | 削除成功＋audit_logs記録／確認キャンセル時は削除されず（削除は別権限でなく操作に含む・破壊性は実行時確認で担保） |
...
| TC-P5-02 | 破壊的削除の実行監査 | U-owner→操作保持者 | vendor を実際に削除（実行時確認OK） | audit_logs に記録（破壊的操作の実行ログ） |
...
- 設定変更系（操作＝登録・編集・削除の一体付与）は**保存→再取得**で結果を確認。破壊的な削除は実行時の確認ダイアログ＋監査で担保（削除は別権限にしない）。
