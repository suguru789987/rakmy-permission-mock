...
## 付録A. feature_key 辞書（全カテゴリ網羅・27カテゴリ53機能）
UIの全カテゴリ・機能と判定キーの完全対応。`view`=一覧画面権限(なし/閲覧)、`ops`=設定画面権限(なし/閲覧/操作)。単一機能カテゴリは `<category_id>`、複数は `<category_id>.<slug>`。slugは実装確定時に意味的命名を踏襲（本表は提案）。
...
| view | `report.pl_daily` | 予実対比（日次） | common | なし/閲覧 |  |
...
| ops | `uicustom.ui` | UIカスタマイズ（ウィジェット/プリセット・DnD編集） | common | なし/閲覧/操作 | 店長以上 |
| ops | `uicustom.tag` | カスタムタグ作成・管理（本部のみ） | common | なし/閲覧/操作 | 会社ロールのみ |
| ops | `label` | ラベル（登録・一覧） | common | なし/閲覧/操作 |  |
| ops | `vendor` | 発注先&仕入商品（一覧・登録・編集） | shop | なし/閲覧/操作 |  |
| ops | `delivery` | 納品書管理（一覧・編集） | common | なし/閲覧/操作 |  |
| ops | `saleslog` | 売上（売上ログ入力・実績） | common | なし/閲覧/操作 |  |
| ops | `inventory.exec` | 棚卸実施（棚卸履歴・実施） | common | なし/閲覧/操作 |  |
| ops | `inventory.product` | 棚卸商品管理 | common | なし/閲覧/操作 |  |
| ops | `pettycash.manage` | 小口現金管理（一覧・登録・締め） | common | なし/閲覧/操作 |  |
| ops | `pettycash.expense_item` | 経費項目設定 | common | なし/閲覧/設定可 | 会社ロールのみ |
| ops | `employees.manage` | 従業員の管理（登録・編集・勤怠・時間記録・人件費・履歴・打刻） | common | なし/閲覧/操作 | 店長以上 |
| ops | `employees.payroll` | 給与設定・履歴 | company | なし/閲覧/操作 | 会社ロールのみ |
| ops | `closing.close` | 締め処理（仮締め・本締め・一括） | company | なし/閲覧/操作 | 会社ロールのみ |
| ops | `closing.fiscal` | 会計年度設定 | company | なし/閲覧/設定可 | 会社ロールのみ |
| ops | `users` | ユーザー（アカウント・招待） | common | なし/閲覧/設定可 |  |
| ops | `transfer` | 店舗間商品移動 | common | なし/閲覧/操作 |  |
| ops | `shop` | 店舗管理（一覧・登録・編集・削除） | company | なし/閲覧/操作 | 会社ロールのみ |
...
| ops | `shopinfo` | 店舗情報 | shop | なし/閲覧/操作 |  |
| ops | `integration` | 連携設定（POS／仕入／勤怠の連携先） | shop | なし/閲覧/設定可 |  |
| ops | `cost.setting` | 費用設定（費目別の費用予算・費目の追加/編集/削除） | shop | なし/閲覧/操作 |  |
| ops | `cost.actual_list` | 費用実績入力・一覧（費目×月の実績入力／年間一覧表示・CSV取込） | shop | なし/閲覧/操作 |  |
| ops | `holiday` | 休業日設定 | shop | なし/閲覧/操作 |  |
| ops | `budget.monthly_wizard` | 月次予算設定（ウィザード：単月費用・売上構成比・曜日別バイアス含む） | shop | なし/閲覧/操作 |  |
| ops | `budget.list` | 予算一覧（月次・年間） | shop | なし/閲覧 |  |
