...
def build_tp():
    ...
    ws.conditional_formatting.add(rng,CellIsRule(operator="equal",formula=['"OK"'],fill=PatternFill("solid",fgColor="D8F0E0")))
    ws.conditional_formatting.add(rng,CellIsRule(operator="equal",formula=['"NG"'],fill=PatternFill("solid",fgColor="FBDCDC")))
    style(ws,hrow,len(hdr),[8,9,20,24,18,24,32,30,28,34,30,28,36,10,14,12,28,16,9,10,12,26],rowh=124)
 ...
 ("6","使う検証データ","検証用データセットのどのシートを使うか。データ不要なケースは「—」","作成時","07（US-03）／03_売上／02_従業員"),
 ("7","操作","実際に行う手順。クリックする場所まで書く","作成時","US-03でログインし、許可画面を順に開く→次に不許可画面をメニュー確認し直URLでも開く"),
 ("8","期待挙動","何が起きればよいか（できること）","作成時","担当2店舗の合計＝3,000,000 になる"),
 ("9","起きてはいけないこと","何が出たらNGか（見えてはいけないもの）。権限の検証は「見えないこと」を確かめる場面が多いため独立させている","作成時","池袋（400万）が混ざらない。池袋三郎の給与80万が出ない"),
 ("10","合否ライン","OK/NGを分ける数値の線。件数・割合・金額で書く。**どの画面で見るかを「画面」列と揃える**","作成時","売上分析の合計＝3,000,000。池袋の混入＝0件（7,000,000ならNG）"),
 ("11","計算根拠","その数値になる理由。計算系のみ。操作系は「—」","作成時","渋谷100万＋新宿200万＝300万。池袋400万が入ると700万になる"),
 ("12","確認ポイント","なぜこれを見るのか、どこで間違えやすいか","作成時","合計が7,000,000なら全社が見えている＝NG。P3を前倒しする根拠"),
 ("13","スクショに写すもの","どの画面のどの瞬間を撮るか。何枚必要かも書く","作成時","売上分析の合計＝3,000,000 と、店舗切替に渋谷・新宿だけが並んだ状態（2枚）"),
 ("14","スクショファイル名","証跡の保存名。検証IDと合わせる","実施時","T-11"),
 ("15","受入条件ID","この検証が担保する受入条件。NGならリリース判断に跳ね返る","作成時","AC-10 AC-11 AC-23"),
 ("16","遷移フローID","対応する遷移フロー（20260803_09）。NGなら遷移の設計に跳ね返る","作成時","F-14"),
 ("17","ヘルプページの該当箇所","顧客に何と説明しているか。NGならヘルプの記述も直す必要がある","作成時","ケース別のやり方（複数の店舗をまとめて見せたい）／担当店舗を設定する（図2）"),
 ("18","実際の値","実施して見えた値・挙動をそのまま書く。判断は書かない","実施時","合計 3,000,000。池袋は切替に出ず"),
 ("19","判定","OK／NG／検証不能。データを投入できない場合は「検証不能」。「問題なし」と書かない","実施時","OK"),
 ("20","実施者","誰が実施したか。証跡として残す","実施時","鈴木"),
 ("21","実施日","いつ実施したか。実装が変わると結果も変わるため必須","実施時","2026-08-20"),
 ("22","備考","NGの詳細、再実施の記録、検証不能の理由など","実施時","1回目NG。修正後の再実施でOK（8/22）"),
    ]
    ...
    for t in ["1. 「画面」を開き、「前提・入力値」が済んでいることを確かめる",
              ...
              "5. 「スクショに写すもの」のとおり撮り、「スクショファイル名」で保存する",
              "6. 「実施者」「実施日」を記入する（実装が変わると結果も変わるため、いつ誰が確かめたかを残す）",
              "7. NG のときは「受入条件ID」「遷移フローID」「ヘルプページの該当箇所」を見て、どこに跳ね返るかを確認し、「備考」に詳細を書く"]:
